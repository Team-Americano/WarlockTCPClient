...
public class InputManager : IInputService
{
    ...
    public RenderQueueEntry[] RenderQueue { get; set; } = new RenderQueueEntry[]
    {
        new RenderQueueEntry {
            Animation = "anim",
            PlayerId = "0",
            TargetPartyInd = new short[] { 0 }
        }
    };
    ...
    public Task DraftActors()
    {
        ...
        {
            Hand = ServiceProvider.PlayerManager.GetHomePlayer().Hand,
            Party = ServiceProvider.PlayerManager.GetHomePlayer().Party,
            Mana = ServiceProvider.PlayerManager.GetHomePlayer().Mana
        };
    ...
    public Task StartCombatPhase(Packet packet)
    {
        ...
        RenderQueue = poco.RenderQueueEntries;
